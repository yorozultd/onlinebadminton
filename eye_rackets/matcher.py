...
with open("preorder.xml","wb") as f1:
    f1.write(pr.content)
preorderxml = open("preorder.xml",encoding="utf-8")
...
to_be_enables =[]
notavailableurl ="http://ec2-35-180-156-95.eu-west-3.compute.amazonaws.com/inventory/not_available/"
nar=requests.get(notavailableurl)
with open("not_available.xml","wb") as f1:
    f1.write(nar.content)
notavailablexml = open("not_available.xml",encoding="utf-8")
tree= ET.parse(notavailablexml);
root = tree.getroot();
products = root.findall("./product")
for product in products : 
    filegtin=str(product.find('gtin').text).strip() 
